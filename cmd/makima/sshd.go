...
//
// This is the narrow version. It binds the mesh address and nothing else, and
// there is no password authentication and no way to add one. A session runs as
// the account named here unless the client asks for another, which is granted
// only by that account's own authorized_keys.
//
...
func sshStatusReport(st localapi.Status) string {
	...
	fmt.Fprintf(&b, "Accepting ssh on %s as %s, or as any account whose own\nauthorized_keys holds the key.\n", s.Addr, s.User)
